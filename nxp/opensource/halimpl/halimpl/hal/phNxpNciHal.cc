...
int phNxpNciHal_core_initialized(uint8_t* p_core_init_rsp_params) {
  ...
  NXPLOG_NCIHAL_D("[%s:%s]", __FILE__, __func__);
    ...
    if ((true == fw_download_success) || (true == setConfigAlways) ||
      ...
      for(int i=0; i< numOfBlocks; i++)
      {
        ...
        if (retlen > 0) {
          status = phNxpNciHal_send_ext_cmd(retlen, buffer);
          if(status != NFCSTATUS_SUCCESS)
            NXPLOG_NCIHAL_E("phNxpNciHal_send_ext_cmd--> status=0x%X", status);
          if ((nfcFL.chipType != pn547C2) && (status == NFCSTATUS_SUCCESS)) {
              status = phNxpNciHal_CheckRFCmdRespStatus();
              if(status != NFCSTATUS_SUCCESS)
                NXPLOG_NCIHAL_E("phNxpNciHal_CheckRFCmdRespStatus--> status=0x%X", status);
              ...
              NXPLOG_NCIHAL_D("RF Settings BLK %u SUCCESS", i+1);
          } else if (status != NFCSTATUS_SUCCESS) {

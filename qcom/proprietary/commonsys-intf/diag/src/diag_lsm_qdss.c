...
int diag_qdss_init()
{
	...
	if (qdss_mask) {
		qdss_periph_info.peripheral_type |= device_mask;
...
static void* qdss_write_thread_mdm(void* param) {
	...
	while (1) {
		...
		if (!qdss_pools_mdm[qdss_curr_write_mdm].data_ready) {
			...
			qdss_in_write_mdm = 0;
			if (qdss_kill_thread) {
				DIAG_LOGD("diag: In %s:Exit write thread for mdm after receiving condition\n", __func__);
				return 0;
			}
		}
	...
	if (remote_mask) {
		...
		for (dev_idx = 1; dev_idx <=  NUM_PROC; dev_idx++) {
			if ((remote_mask & (1 << (dev_idx - 1))) &&
				(qdss_periph_info.peripheral_type & (1 << dev_idx))) {
				if (qdss_mask & DIAG_CON_MPSS) {

	...
	for (i = 0; i < num_bytes; i++) {
		if (buf[i] == CONTROL_CHAR) {
			if (hdlc_disabled && (end == start) && !cmd_size) {
				packet_len = (uint16_t)((*(uint16_t *)(buf + i + 2)));
			} else {
		...
		if (hdlc_disabled) {
			i = start + packet_len + (DIAG_NON_HDLC_HEADER_SIZE - 1);
			end = start + packet_len + DIAG_NON_HDLC_HEADER_SIZE;
		} else {
			end++;
		}
	}
...
void flush_buffer(int signal)
{
	...
	if (signal) {
		...
		if (stat(pid_file, &pid_stat) == 0)
			...
			DIAG_LOGE("diag: PID file is not present: %s, err: %d\n", pid_file, errno);

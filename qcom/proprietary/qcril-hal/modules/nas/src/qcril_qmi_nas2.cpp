...
extern void enable5gResponse(RIL_Token token, RIL_Errno err_num);
extern void enable5gOnlyResponse(RIL_Token token, RIL_Errno err_num);
extern void disable5gResponse(RIL_Token token, RIL_Errno err_num);
extern void query5gStatusResponse(RIL_Token token, RIL_Errno err_num,
        five_g_status status);
extern void enableEndcResponse(RIL_Token token, RIL_Errno err_num);
extern void queryEndcStatusResponse(RIL_Token token, RIL_Errno err_num,
        endc_status status);
...
)
{
  ...
  enable5gResponse(params_ptr->t, ril_req_res);
...
)
{
  ...
  using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
  enable5gOnlyResponse(params_ptr->t, ril_req_res);
...
)
{
  ...
  using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
  disable5gResponse(params_ptr->t, ril_req_res);
...
)
{
  ...
  do
  {
      ...
      if (current_mode_pref_valid)
      {
          is_5g_enabled = current_mode_pref & QMI_NAS_RAT_MODE_PREF_NR5G;
      }
      else
      {
          QCRIL_LOG_ERROR("Cannot obtain current mode preference.");
  ...
  using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
  query5gStatusResponse(params_ptr->t, ril_req_res, (five_g_status) is_5g_enabled);
...
)
{
  ...
  if ( params_ptr->datalen == 0 || params_ptr->data == NULL  )
  {
      QCRIL_LOG_ERROR("Invalid enable endc request");
      using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
#ifndef QMI_RIL_UTF
      enableEndcResponse(static_cast<qcril_event_context*>(params_ptr->t)->t, ril_req_res);
#endif
      return;
  }

  QCRIL_LOG_INFO("request enable_endc: %d", *(uint32 *)params_ptr->data);

  memset(&set_endc_config_req_msg, 0, sizeof(set_endc_config_req_msg));
  switch(*(uint32 *)params_ptr->data){
      case 0:
      ...
      case 1:
          ...
          break;
      default:
          QCRIL_LOG_ERROR("Invalid endc request");
  ...
  if ( qmi_client_nas_send_sync (QMI_NAS_SET_ENDC_CONFIG_REQ_MSG_V01,
                                   &set_endc_config_req_msg,
                                   sizeof(set_endc_config_req_msg),
                                   &set_endc_config_resp_msg,
                                   sizeof(set_endc_config_resp_msg),
                                   ModemEndPoint::SYNC_REQ_UNRESTRICTED_TIMEOUT) != QMI_NO_ERR)
  {
  ...
  {
      QCRIL_LOG_DEBUG("enable_endc resp error from modem:%d", set_endc_config_resp_msg.resp.error);
      if(set_endc_config_resp_msg.resp.result == QMI_RESULT_FAILURE_V01)
      {
          ril_req_res = RIL_E_GENERIC_FAILURE;
      }
      else
...
)
{
  ...
  if ( qmi_client_nas_send_sync (QMI_NAS_GET_ENDC_CONFIG_REQ_MSG_V01,
                                   NULL,
                                   0,
  ...
  {
      QCRIL_LOG_DEBUG("endc_status resp error from modem: %d",get_endc_config_resp_msg.resp.error);

      if(get_endc_config_resp_msg.resp.result == QMI_RESULT_FAILURE_V01)
      {
          ril_req_res = RIL_E_GENERIC_FAILURE;
      }
      else
      {
          if (get_endc_config_resp_msg.enable_endc_valid)
          {
            switch(get_endc_config_resp_msg.enable_endc)
            {
                case 0:
                    is_endc_enabled = false;
                    ril_req_res = RIL_E_SUCCESS;
                    break;
                case 1:
                    is_endc_enabled = true;
                    ril_req_res = RIL_E_SUCCESS;
                    break;
                default:
            ...
            QCRIL_LOG_INFO("endc status response from modem:%d",get_endc_config_resp_msg.enable_endc);
          }
      }
  }
  ...
  using namespace vendor::qti::hardware::radio::qtiradio::V1_0::implementation;
#ifndef QMI_RIL_UTF
  queryEndcStatusResponse(static_cast<qcril_event_context*>(params_ptr->t)->t,
                            ril_req_res, (endc_status)is_endc_enabled);
#endif
  QCRIL_LOG_FUNC_RETURN();
}
    ...
    if ( RIL_E_SUCCESS == res )
    {
          ...
          switch (qcril_qmi_nas_get_5g_mode_pref())
          {
              case FIVE_G_MODE_INCLUSIVE:
                  set_system_selection_preference_req_msg.mode_pref |= QMI_NAS_RAT_MODE_PREF_NR5G;
                  break;
...
)
{
  ...
  else
  {
      ...
      if ( mode_pref_valid )
      {
          // filter out 5G from the mode preference reported to Android
          mode_pref &= (~QMI_NAS_RAT_MODE_PREF_NR5G);
          switch ( mode_pref )
          {

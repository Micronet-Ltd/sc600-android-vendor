...
QCRIL_ANDROID_DECLARE_REQUEST(RIL_REQUEST_ENABLE_ENDC, int *, void);
QCRIL_ANDROID_DEFINE_REQUEST(RIL_REQUEST_ENABLE_ENDC);

QCRIL_ANDROID_DECLARE_REQUEST(RIL_REQUEST_QUERY_ENDC_STATUS, void, void);
QCRIL_ANDROID_DEFINE_REQUEST(RIL_REQUEST_QUERY_ENDC_STATUS);
...
NasModule::NasModule() {
  ...
  mMessageHandler = {
    ...
    LEGACY_HANDLER(RIL_REQUEST_ENABLE_ENDC, NasModule::handleEnableEndc),
    LEGACY_HANDLER(RIL_REQUEST_QUERY_ENDC_STATUS, NasModule::handleQueryEndcStatus),
...
void NasModule::init()
{
  ...
  QCRIL_ANDROID_REGISTER_REQUEST(RIL_REQUEST_ENABLE_ENDC, callback);
  QCRIL_ANDROID_REGISTER_REQUEST(RIL_REQUEST_QUERY_ENDC_STATUS, callback);
...
void NasModule::handleEnableEndc(std::shared_ptr<RilRequestMessage> shared_msg) {
    Log::getInstance().d("[NasModule]: Handling msg = " + shared_msg->dump());
    qcril_request_return_type ret_ptr;
    qcril_qmi_nas_request_enable_endc(&shared_msg->get_params(), &ret_ptr);
    Dispatcher::getInstance().informMessageCallbackFinished(shared_msg);
}

void NasModule::handleQueryEndcStatus(std::shared_ptr<RilRequestMessage> shared_msg) {
    Log::getInstance().d("[NasModule]: Handling msg = " + shared_msg->dump());
    qcril_request_return_type ret_ptr;
    qcril_qmi_nas_request_query_endc_status(&shared_msg->get_params(), &ret_ptr);
    Dispatcher::getInstance().informMessageCallbackFinished(shared_msg);
}

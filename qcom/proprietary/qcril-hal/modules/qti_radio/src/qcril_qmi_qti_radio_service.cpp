...
Return<void> QtiRadioImpl::enableEndc (int32_t serial, bool enable) {
    ...
    uint32_t* endc_enable = (uint32_t*)qcril_malloc(sizeof(uint32_t));
    if(endc_enable != nullptr) {
        *endc_enable = BOOL_TO_INT(enable);

        RIL_Token token = qcril_qmi_qti_convert_radio_token_to_ril_token(serial);
        if (token != nullptr) {
            onRequest(RIL_REQUEST_ENABLE_ENDC, endc_enable, sizeof(uint32_t), token);
        }
        qcril_free(endc_enable);
    }
    return Void();
}

#ifndef QMI_RIL_UTF
void QtiRadioImpl::enableEndcResponse(RIL_Token token, RIL_Errno err_num) {
    sp<V2_3::IQtiRadioResponse> respCb = getResponseCallbackV2_3();
    if (respCb != nullptr) {
        int serial = qcril_qmi_qti_free_and_convert_ril_token_to_radio_token(token);
        QCRIL_LOG_DEBUG ("QtiRadioImpl::enableEndcResponse serial=%d", serial);
        ...
        QCRIL_LOG_ERROR("QtiRadioImpl::enableEndcResponse respCb NULL");
    }
}
#endif
...
Return<void> QtiRadioImpl::queryEndcStatus (int32_t serial) {
    QCRIL_LOG_ERROR ("QtiRadioImpl::queryEndcStatus, serial=%d", serial);
    RIL_Token token = qcril_qmi_qti_convert_radio_token_to_ril_token(serial);
    if (token != nullptr) {
        onRequest(RIL_REQUEST_QUERY_ENDC_STATUS, NULL, 0, token);
    }
    return Void();
}
#endif
...
#ifndef QMI_RIL_UTF
void QtiRadioImpl::queryEndcStatusResponse(RIL_Token token, RIL_Errno err_num,
        endc_status status) {
    sp<V2_3::IQtiRadioResponse> respCb = getResponseCallbackV2_3();
    if (respCb != nullptr) {
        int serial = qcril_qmi_qti_free_and_convert_ril_token_to_radio_token(token);
        QCRIL_LOG_DEBUG ("QtiRadioImpl::queryEndcStatusResponse serial=%d", serial);
        Return<void> ret = respCb->onEndcStatusResponse(serial,
                static_cast<uint32_t>(err_num), convert_endc_status(status));
        if (!ret.isOk()) {
            QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
        }
    }
    else {
        QCRIL_LOG_ERROR("QtiRadioImpl::queryEndcStatusResponse respCb NULL");
    }
}
#endif
...
#ifndef QMI_RIL_UTF
V2_3::EndcStatus convert_endc_status(endc_status status) {
    switch (status) {
        case ENDC_STATUS_DISABLED:
            return V2_3::EndcStatus::DISABLED;
        case ENDC_STATUS_ENABLED:
            return V2_3::EndcStatus::ENABLED;
        default:
            return V2_3::EndcStatus::INVALID;
    }
}
#endif
...
#ifndef QMI_RIL_UTF
void enableEndcResponse(RIL_Token token, RIL_Errno err_num) {
    if (qtiRadioService != nullptr) {
        qtiRadioService->enableEndcResponse(token, err_num);
    }
}

void queryEndcStatusResponse(RIL_Token token, RIL_Errno err_num, endc_status status) {
    if (qtiRadioService != nullptr) {
        qtiRadioService->queryEndcStatusResponse(token, err_num, status);
    }
}
#endif

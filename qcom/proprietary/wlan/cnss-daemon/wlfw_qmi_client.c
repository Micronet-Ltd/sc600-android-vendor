...
#define XO_CAL_FILE		"/data/vendor/xo_cal_data.bin"
#define FALSE			1
...
static int wlfw_send_ind_register_req(uint64_t *fw_status)
{
	...
	if (gdata.instance_id == NAPIER_ID || gdata.instance_id == HAWKEYE_ID) {
...
static int wlfw_send_cal_report_req(void)
{
	...
	if (gdata.instance_id == ADRASTEA_ID) {
		if (wlfw_read_file(XO_CAL_FILE, &pcal) > 0) {
...
int wlfw_save_file(const char *filename, unsigned char *data, int len)
{
	int len_written = 0;
	FILE *fp = NULL;
				    ...
				    unsigned int total_size)
{
	...
	if (remaining == 0 && (resp.end_valid && resp.end)) {
		...
		wlfw_save_file(filename, p_cal_data, total_size);
		goto end;
	} else {
					 ...
					 const char *file_name)
{
	...
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_DATA_REQ_V01, 0);
	while (remaining && resp.end == 0) {
		rc = qmi_client_send_msg_sync(gdata.clnt_handler,
	...
	if (remaining == 0 && (resp.end_valid && resp.end)) {
		...
		wlfw_save_file(filename, p_qdss_trace_data, total_size);
	} else {
...
fail:
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_DATA_REQ_V01, rc);
					 ...
					 unsigned long long option)
{
	...
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_MODE_REQ_V01,0);
	rc = qmi_client_send_msg_sync(gdata.clnt_handler,
	...
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_MODE_REQ_V01, rc);
	return rc;
}
...
int wlfw_send_qdss_trace_config_download_req(void)
{
	...
	temp = buffer;
	remaining = file_len;
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_CONFIG_DOWNLOAD_REQ_V01, 0);
	while (remaining) {
		req.total_size_valid = 1;
		req.total_size = file_len;
		req.seg_id_valid = 1;
...
out:
	daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_CONFIG_DOWNLOAD_REQ_V01, rc);
	free(buffer);
	buffer = NULL;
	return rc;
read_fail:
...
static void wlfw_handle_xo_cal_ind(void *ind_buf, unsigned int ind_buf_len)
{
	...
	wlfw_save_file(XO_CAL_FILE, &decoded_ind_buf.xo_cal_data,
		       sizeof(decoded_ind_buf.xo_cal_data));
}
			    ...
			    void *ind_cb_data)
{
	...
	case QMI_WLFW_QDSS_TRACE_SAVE_IND_V01:
		daemon_qmihist_record(QMI_WLFW_QDSS_TRACE_SAVE_IND_V01, 0);

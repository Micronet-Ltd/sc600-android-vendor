        ...
        unsigned char *iebuf)
{
    ...
    if (!(scaninfo->srp.client_srp_disallowed)) {
        pchlist->ch[chn_idx].num_wnw_client_srp_allowed++;
    }

    if (!(scaninfo->srp.client_obsspd_disallowed)) {
        ...
        ICM_INFO_T *picm)
{
    ...
    } else if (picm->band == ICM_BAND_DUAL) {
        picm->sbmgmt_ctx.req_scanband = SCAN_BAND_ALL;
    }

...
int uxe_disabled() {
    ...
    property_get("vendor.iop.enable_uxe", property, "1");
        ...
        switch (cmd) {
            case UXENGINE_CMD_PERFLOCK_TRIGGER:
            {
                ...
                if(msg->opcode == UXE_TRIGGER)
                {
                    ...
                    uba_return = get_preferred_apps(final_out, 0, NULL, -1);
                    ...
                    for (int time_slot_iter = 1; time_slot_iter <= 4; time_slot_iter++) {
                        for(int week_day_iter = 0; week_day_iter < 2; week_day_iter++) {
                            uba_return = get_preferred_apps(final_out, 1, weekday[week_day_iter], time_slot_iter);
...
int iop_server_init() {
    ...
    if (uxe_disabled()) {
        ...
        uxe_prop_disable = 0;
        property_get("vendor.iop.uxe_trigger_freq", property, "1");

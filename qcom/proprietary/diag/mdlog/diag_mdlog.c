...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "f:m:l:o:p:s:w:n:t:x:g:q:j:cdkebrahi", longopts, NULL))
			!= -1) {
...
int main(int argc, char *argv[])
{
	...
	z = 1;
	err = ioctl(diag_fd_temp, DIAG_IOCTL_REMOTE_DEV, &remote_mask, 0, NULL, 0, NULL, NULL);
	if (err != 1)
	{
		if (diag_fd_temp != DIAG_INVALID_HANDLE) {
		...
		DIAG_LOGE("diag: error in getting remote device mask\n");
		exit(0);
	}
	while(remote_mask) {
		if(remote_mask & 1 ) {

...
static void *aggregator_thread_loop(void *st_session)
{
    ...
    while (!st_ses->exit_aggregator_loop) {
        ...
        list_for_each_safe(node, tmp_node, &st_ses->second_stage_list) {
            ...
            if (st_sec_stage->ss_session->det_status == KEYWORD_DETECTION_SUCCESS)
        ...
        list_for_each_safe(node, tmp_node, &st_ses->second_stage_list) {
            ...
            pthread_rwlock_rdlock(st_sec_stage->ss_session->pcm_rw_lock);
            if ((st_sec_stage->ss_info->sm_detection_type ==
                    ST_SM_TYPE_USER_VERIFICATION) &&
                (det_status & USER_VERIFICATION_PENDING)) {
                handle_vop_pending_detection(st_sec_stage->ss_session, det_status,
                    kw_det_buff_sz);
            } else if ((st_sec_stage->ss_info->sm_detection_type ==
                    ST_SM_TYPE_KEYWORD_DETECTION) &&
                (det_status & KEYWORD_DETECTION_PENDING)) {
                handle_cnn_pending_detection(st_sec_stage->ss_session, det_status);
            }
            pthread_rwlock_unlock(st_sec_stage->ss_session->pcm_rw_lock);
        }
        ...
        if (!IS_SS_DETECTION_PENDING(det_status)) {
            ...
            if (IS_SS_DETECTION_SUCCESS(det_status)) {
                ...
                callback = st_ses->callback;
                ALOGD("%s: Second stage detected successfully, calling client callback",
                    __func__);
                ...
                /*
                 * The client could unload the sound model during the callback, which
                 * would join this thread and wait for this thread exit as part of
                 * st_session_deinit() with st_session_lock held. By this time, the
                 * state is also moved to idle. To avoid deadlock, upon return from
                 * client callback, try acquiring lock only if not in idle state, else
                 * exit right away.
                 */
                do {
                    lock_status = pthread_mutex_trylock(&st_ses->lock);
                } while (lock_status && (st_ses->current_state != idle_state_fn));

                if (st_ses->current_state == idle_state_fn) {
                    ALOGV("%s:[%d] client unloaded after callback, lock status %d",
...
static void init_det_event_aggregator(st_session_t *st_ses)
{
    ...
    pthread_mutex_init(&(st_ses->ss_det_lock), NULL);
    pthread_condattr_init(&attr);
    pthread_condattr_setclock(&attr, CLOCK_MONOTONIC);
    pthread_cond_init(&(st_ses->ss_det_cond), &attr);
...
static void destroy_det_event_aggregator(st_session_t *st_ses)
{
    ...
    pthread_mutex_lock(&st_ses->ss_det_lock);
    pthread_cond_signal(&st_ses->ss_det_cond);
    pthread_mutex_unlock(&st_ses->ss_det_lock);
    status = pthread_join(st_ses->aggregator_thread, NULL);
    if (status)
        ALOGE("%s: Error joining aggregator thread. status = %d",
            __func__, status);
    pthread_cond_destroy(&(st_ses->ss_det_cond));
    pthread_mutex_destroy(&(st_ses->ss_det_lock));
}

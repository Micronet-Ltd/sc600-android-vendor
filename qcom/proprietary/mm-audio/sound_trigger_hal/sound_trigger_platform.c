...
#define ST_PARAM_KEY_TRANSIT_TO_ADSP_ON_BATTERY_CHARGING "transit_to_adsp_on_battery_charging"
...
)
{
    ...
    err = str_parms_get_str(parms, ST_PARAM_KEY_TRANSIT_TO_ADSP_ON_BATTERY_CHARGING,
                            str_value, sizeof(str_value));
    if (err >= 0) {
...
static int query_stdev_platform(sound_trigger_device_t *stdev,
                                 const char *snd_card_name,
                                 char *mixer_path_xml)
{
    ...
    } else if (strstr(snd_card_name, "qcs405-csra")) {
        char *tmp = NULL;
        char *csra_tag = NULL;
        char *snd_card_name_tmp = strdup(snd_card_name);

        if (!snd_card_name_tmp)
            return -ENOMEM;

        strlcpy(mixer_path_xml, MIXER_PATH_FILE_NAME,
                sizeof(MIXER_PATH_FILE_NAME));

        csra_tag = strtok_r(snd_card_name_tmp, "-", &tmp);
        while (csra_tag != NULL) {
            csra_tag = strtok_r(NULL, "-", &tmp);
            if ((csra_tag != NULL) && strstr(csra_tag, "csra"))
                break;
        }

        if (!csra_tag) {
            free(snd_card_name_tmp);
            return -ENOMEM;
        }

        strlcat(mixer_path_xml, "_", MIXER_PATH_MAX_LENGTH);
        strlcat(mixer_path_xml, csra_tag, MIXER_PATH_MAX_LENGTH);
        free(snd_card_name_tmp);
    } else {
        strlcpy(mixer_path_xml, MIXER_PATH_FILE_NAME,
                         sizeof(MIXER_PATH_FILE_NAME));
...
void *platform_stdev_init(sound_trigger_device_t *stdev)
{
    ...
    ret = query_stdev_platform(stdev, snd_card_name, mixer_path_xml);
    if (ret) {
        ALOGE("%s: ERROR. Failed to update platform, status %d, aborting.",
                __func__, ret);
        goto cleanup;
    }
...
)
{
    ...
    case AUDIO_DEVICE_IN_BUILTIN_MIC:
        if (ST_EXEC_MODE_CPE == exec_mode) {
            ...
            if (!check_haven_license(my_data, my_data->codec_backend_cfg.lpi_enable,

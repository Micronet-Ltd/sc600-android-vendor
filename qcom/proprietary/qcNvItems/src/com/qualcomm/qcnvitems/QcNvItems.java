...
public class QcNvItems implements IQcNvItems {
    ...
    public void setNvFactoryData3IByte(byte[] bresult) throws IOException, InvalidParameterException {
        vLog(String.format("setNvFactoryData3I()"));
        if (bresult == null || bresult.length != 128) {
            throw new InvalidParameterException();
        }
        NvFactoryData3I o = new QcNvItemTypes.NvFactoryData3I(bresult);
        doNvWrite(o, QcNvItemIds.NV_FACTORY_DATA_3_I, "000");
        Log.e(LOG_TAG, "NV_VENDOR_YULONG_NV_FACTORY_DATA_INFO_I_Byte:" + bresult);
    }
    
    public void setNvFactoryData3I(String result) throws IOException, InvalidParameterException {
        vLog(String.format("setNvFactoryData3I()"));
        ...
        NvFactoryData3I o = new QcNvItemTypes.NvFactoryData3I(bresult);
        doNvWrite(o, QcNvItemIds.NV_FACTORY_DATA_3_I, "000");
        Log.e(LOG_TAG, "NV_VENDOR_YULONG_NV_FACTORY_DATA_INFO_I:" + bresult);
    }
    
    public String getNvFactoryData3I() throws IOException {

        vLog(String.format("getNvFactoryData3I"));

        String result=null;

		byte[] bresult = doNvRead(QcNvItemIds.NV_FACTORY_DATA_3_I);
		result =  new String(bresult);
		Log.e(LOG_TAG, "NvFactoryData3I:" + result);

        return result;
    }
    
    public byte[] getNvFactoryData3IByte() throws IOException {
        vLog(String.format("getNvFactoryData3IByte"));
		byte[] bresult = doNvRead(QcNvItemIds.NV_FACTORY_DATA_3_I);
		Log.e(LOG_TAG, "NvFactoryData3IByte:" + bresult);
...
//         }
    ...
    public String getSNNumber() throws IOException {

        vLog(String.format("getSNNumber()"));

        String sn = null;

        byte[] bSN1 = doNvRead(QcNvItemIds.NV_FACTORY_DATA_1_I);
        sn =  new String(bSN1);
        Log.e(LOG_TAG, "NV_FACTORY_DATA_1_I:" + sn);

        return sn;
    }

   public String getMeid() throws IOException {
   
        vLog(String.format("getMeid()"));
   
        String Meid = null;
   
        byte[] meid = doNvRead(QcNvItemIds.NV_MEID_I);
        Meid =  new String(meid);
        Log.e(LOG_TAG, "NV_MEID_I:" + Meid);
   
        return Meid;
   }

   //xiaciping add for set MEID --NV#1943 and  IMEI --NV#550
    public void setMEID(String meid) throws IOException, InvalidParameterException {
        vLog("setMEID() MEID:" + meid);
        ...
        NvMeidType meid_t;
        meid_t = new NvMeidType();
	 meid_t.setMeid(meid);
	 
        doNvWrite(meid_t, QcNvItemIds.NV_MEID_I, DEFAULT_SPC);
    }

    public String getIMEI() throws IOException {
   
        vLog(String.format("getIMEI()"));
   
        String IMEI = null;
   
        byte[] imei = doNvRead(QcNvItemIds.NV_UE_IMEI_I);
        IMEI =  new String(imei);
        if( (IMEI != null) && (IMEI.length() == 16)) {
          Log.e(LOG_TAG, "NV_UE_IMEI_I:" + IMEI.substring(0,15));
    ...
    public String getIMEI2() throws IOException {
   
        vLog(String.format("getIMEI2()"));
   
        String IMEI = null;
   
        byte[] imei = doNvRead(QcNvItemIds.NV_UE_IMEI2_I);
        IMEI =  new String(imei);
        if( (IMEI != null) && (IMEI.length() == 15)) {
          Log.e(LOG_TAG, "NV_UE_IMEI2_I:" + IMEI);
          return IMEI;
        } else {
          return null;
        }
       ...
       public void setIMEI(String imei) throws IOException, InvalidParameterException {
        vLog("setIMEI() IMEI:" + imei);
        ...
        NvIMEIType imei_t;
        imei_t = new NvIMEIType();
	 imei_t.setImei(imei);

        doNvWrite(imei_t, QcNvItemIds.NV_UE_IMEI_I, DEFAULT_SPC);
    }

       public void setSarToModem(int sar) throws IOException, InvalidParameterException {
        vLog("setSarToModem:" + sar);

        NvInteger sar_t;
        sar_t = new NvInteger();
        sar_t.mVal = sar;
        doNvWrite(sar_t, QcNvItemIds.NV_SET_SAR_RF_STATE, DEFAULT_SPC);
    }
   //End xiaciping add 
   

   public String getMEIDCD() throws IOException {

        vLog(String.format("getMEIDCD()"));
        String meidcdStr = null;

        try {
        	byte[] bMeidCd = doNvRead(QcNvItemIds.NV_MEID_CD_I);
        	meidcdStr = IccUtils.bytesToHexString(bMeidCd);
        	meidcdStr = meidcdStr.substring(meidcdStr.length() - 1);
	} catch (Exception e) {
		// TODO: handle exception
		meidcdStr = "";
        }
	Log.e(LOG_TAG, "NV_MEID_CD_I:" + meidcdStr);

        return meidcdStr;
    }

   	public String getCUReferenceNumber() throws IOException {

        vLog(String.format("getCUReferenceNumber()"));

        String cuReferenceNumber = null;

		byte[] bcuReferenceNumber = doNvRead(QcNvItemIds.NV_VENDOR_TCL_CU_REFERENCE_NUMBER_I);
		cuReferenceNumber = new String(bcuReferenceNumber);
		Log.e(LOG_TAG, "NV_VENDOR_TCL_CU_REFERENCE_NUMBER_I:" + cuReferenceNumber);

        return cuReferenceNumber;
    }
   
   	public void setSMSAutoRegisterInfo(String ssmsAutoRegisterInfo, int itemId) throws IOException, InvalidParameterException {
   		vLog(String.format("setSMSAutoRegisterInfo()"));
       	if (ssmsAutoRegisterInfo == null || ssmsAutoRegisterInfo.length() > 124) {
       		throw new InvalidParameterException();
       	}
       	byte[] bsmsAutoRegisterInfo = ssmsAutoRegisterInfo.getBytes();
		NvSmsAuto o = new QcNvItemTypes.NvSmsAuto(bsmsAutoRegisterInfo);
		doNvWrite(o, itemId, "000");
		Log.e(LOG_TAG, "NV_VENDOR_YULONG_SMS_AUTO_REGISTER_INFO_I:" + bsmsAutoRegisterInfo);
   	}
   
   	public String getSMSAutoRegisterInfo(int itemId) throws IOException {
   		vLog(String.format("getSMSAutoRegisterInfo()"));
   		String ssmsAutoRegisterInfo = null;
   		byte[] bsmsAutoRegisterInfo = doNvRead(itemId);
   		ssmsAutoRegisterInfo =  new String(bsmsAutoRegisterInfo);
   		Log.e(LOG_TAG, "NV_VENDOR_YULONG_SMS_AUTO_REGISTER_INFO_I:" + ssmsAutoRegisterInfo);
   		return ssmsAutoRegisterInfo;
   	}

    public String getBtAddr() throws IOException {
   
        vLog(String.format("getBtAddr()"));
   
        String BtAddr = null;
   
        byte[] btaddr = doNvRead(QcNvItemIds.NV_UE_BT_ADDR_I);
        BtAddr =  new String(btaddr);
        if(BtAddr != null && BtAddr.length() >= 17) {
          Log.e(LOG_TAG, "NV_UE_BT_ADDR_I:" + BtAddr);
          return BtAddr.substring(0,17);
        } else {
          return null;
        }
    }

    public String getWlanAddr() throws IOException {
   
        vLog(String.format("getWlanAddr()"));
   
        String WlanAddr = null;
   
        byte[] wlanaddr = doNvRead(QcNvItemIds.NV_UE_WLAN_ADDR_I);
        WlanAddr =  new String(wlanaddr);
        if(WlanAddr != null && WlanAddr.length() >= 17) {
          Log.e(LOG_TAG, "NV_UE_WLAN_ADDR_I:" + WlanAddr);
          return WlanAddr.substring(0,17);
        } else {
          return null;
        }
    }

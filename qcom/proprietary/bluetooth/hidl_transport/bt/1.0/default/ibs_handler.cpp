...
#define RX_THREAD_USAGE_TIMEOUT  (5000)
...
void IbsHandler::InitInt()
{
  ...
  memset(&hidl_rx_state_machine_, 0, sizeof(hidl_rx_state_machine_));
...
IbsHandler::~IbsHandler()
{
  ...
  if (hidl_rx_state_machine_.timer_created == true) {
    timer_delete(hidl_rx_state_machine_.timer_id);
    hidl_rx_state_machine_.timer_created = false;
  }
...
void IbsHandler::RxThreadTimeOut(union sigval /* sig */)
{
#ifdef DETECT_SPURIOUS_WAKE
  std::unique_lock<std::mutex> guard(ibs_mutex_);
  ALOGE("%s:Rx thread struck detected",__func__);
  ...
  UartController* uart_controller = static_cast<UartController*>(instance_->controller);
  uart_controller->SsrCleanup(REASON_RX_THREAD_STRUCK);
  return;
#endif /* DETECT_SPURIOUS_WAKE */
}

void IbsHandler::StopRxThreadUsageTimer()
{
#ifdef DETECT_SPURIOUS_WAKE
  int status;
  struct itimerspec ts;
  ...
  if(hidl_rx_state_machine_.timer_created == true) {
    ...
    ts.it_interval.tv_sec = 0;
    ts.it_interval.tv_nsec = 0;
    status = timer_settime(hidl_rx_state_machine_.timer_id, 0, &ts, 0);
    if(status == -1)
      ALOGE("%s:Failed to stop set Rx thread timer",__func__);
    else if(status == 0)
      ALOGV("%s: Rxthread timer Stopped",__func__);
  }
#endif /* DETECT_SPURIOUS_WAKE */
}

void IbsHandler::StartRxThreadUsageTimer()
{
#ifdef DETECT_SPURIOUS_WAKE
  int status;
  struct itimerspec ts;
  ...
  ALOGV("%s", __func__);
  if (hidl_rx_state_machine_.timer_created == false) {
    se.sigev_notify_function = (void (*)(union sigval))RxThreadTimeOut;
    se.sigev_notify = SIGEV_THREAD;
    se.sigev_value.sival_ptr = &hidl_rx_state_machine_.timer_id;
    se.sigev_notify_attributes = NULL;

    status = timer_create(CLOCK_MONOTONIC, &se, &hidl_rx_state_machine_.timer_id);
    if (status == 0)
      hidl_rx_state_machine_.timer_created = true;
  }

  if (hidl_rx_state_machine_.timer_created == true) {
    hidl_rx_state_machine_.timeout_ms = RX_THREAD_USAGE_TIMEOUT;
    ts.it_value.tv_sec = hidl_rx_state_machine_.timeout_ms / 1000;
    ts.it_value.tv_nsec = 1000000 * (hidl_rx_state_machine_.timeout_ms % 1000);
    ts.it_interval.tv_sec = 0;
    ts.it_interval.tv_nsec = 0;
...
#endif
  /* It is possible BT Stack was shut down, and it could lead to Rx thread stuck,
   * and can result in false detection of spurious wake, so reset is_spurious_wake flag
   * if already set .
   */
  UartController* uart_controller = static_cast<UartController*>(instance_->controller);
  if (uart_controller->is_spurious_wake) {
    ALOGW("%s: resetting is_spurious_wake as false detection of"
        " spurious wake condition", __func__);
    uart_controller->is_spurious_wake = false;
  }

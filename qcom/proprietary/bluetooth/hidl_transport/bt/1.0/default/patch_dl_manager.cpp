...
int PatchDLManager::PerformChipInit()
{
  ...
  if (strcmp(emb_wp_mode, "true") == 0) {
    ALOGV("%s: ro.vendor.bluetooth.emb_wp_mode is true", __func__);
    is_emb_wp_mode = true;
  }
...
int PatchDLManager::GetTlvFile(const char *file_path, const char* alt_file_path)
{
  ...
  if (!(file_path && (pFile = fopen( file_path, "r" )))) {
    ...
    if (!(alt_file_path && (pFile = fopen(alt_file_path, "r")))) {
      ALOGE("%s File Opening from alternate path: Fail", alt_file_path);
      return -1;
    } else {
      ALOGI("File open %s succeded", alt_file_path);
    }
  } else {
    ALOGI("File open %s succeded", file_path);
  }
...
int PatchDLManager::GetAddOnFeatureList()
{
  ...
  if ((param_len > HCI_EVENT_PARAMS)  && (cmd_status == 0x00)) {
    add_on_feat_mask_len = param_len - HCI_EVENT_PARAMS;
    ALOGI("%s: add_on_feat_mask_len 0x%x", __func__,add_on_feat_mask_len);
    memcpy(add_on_features.features, &rsp[FEAT_MASK_BYTES], add_on_feat_mask_len);
  }
...
void PatchDLManager::ReadScramblingSupport()
{
  int size, err = 0;
  unsigned char cmd[HCI_MAX_CMD_SIZE];
  unsigned char rsp[HCI_MAX_EVENT_SIZE];
  unsigned char temp_wait_vsc_evt;
  uint8_t number_of_scrambling_freqs;
  uint8_t* scrambling_freqs;
  char value[PROPERTY_VALUE_MAX] = { '\0' };
  hci_command_hdr *cmd_hdr;
  ...
  cmd_hdr = (hci_command_hdr*)(cmd + 1);
  cmd[0]  = HCI_COMMAND_PKT;
  cmd_hdr->opcode =  cmd_opcode_pack(HCI_VENDOR_CMD_OGF, HCI_SPLIT_A2DP_OPCODE);
  cmd_hdr->plen     = 1;
  cmd[4]  = HCI_VSC_GET_SCRAMBLING_FREQS;

  /* Total length of the packet to be sent to the Controller */
  size = (HCI_CMD_IND + HCI_COMMAND_HDR_SIZE + cmd[PLEN]);
  ALOGI("%s: Sending GET_SCRAM_FREQUENCIES_CMD", __func__);
  ...
  if (err < 0) {
    ALOGE("%s: Failed to read rsp for scram freq cmd", __func__);
    return;
  }

  if (( rsp[4] == HCI_SPLIT_A2DP_OPCODE ) &&
      ( rsp[7] == HCI_VSC_GET_SCRAMBLING_FREQS )) {
    ALOGD("%s: scrambling feature supported!!", __FUNCTION__);

    number_of_scrambling_freqs =  rsp[8];
    scrambling_freqs = &rsp[9];

    ALOGD("%s number of scrambling supported freqs %d", __func__,
            number_of_scrambling_freqs);

    for ( uint8_t i = 0; i < number_of_scrambling_freqs; i++) {
      switch (scrambling_freqs[i]) {
        case AUDIO_CODEC_SAMPLE_RATE_44100:
          strlcat(value, "441.", SCRAMBLING_FREQS_SIZE);
          break;
        case AUDIO_CODEC_SAMPLE_RATE_48000:
          strlcat(value, "48.", SCRAMBLING_FREQS_SIZE);
          break;
        case AUDIO_CODEC_SAMPLE_RATE_88200:
          strlcat(value, "882.", SCRAMBLING_FREQS_SIZE);
          break;
        case AUDIO_CODEC_SAMPLE_RATE_96000:
          strlcat(value, "96.", SCRAMBLING_FREQS_SIZE);
          break;
        case AUDIO_CODEC_SAMPLE_RATE_176400:
          strlcat(value, "1764.", SCRAMBLING_FREQS_SIZE);
          break;
        case AUDIO_CODEC_SAMPLE_RATE_192000:
          strlcat(value, "192.", SCRAMBLING_FREQS_SIZE);
          break;
      }
    }

    if(number_of_scrambling_freqs) {
      if(!property_set("persist.vendor.bt.soc.scram_freqs", value)) {
        ALOGD("%s:persist.vendor.bt.soc.scram_freqs set to %s",
               __func__, value);
      }
    } else {
      // reset the property
      property_set("persist.vendor.bt.soc.scram_freqs", "");
    }
  }
}
...
int PatchDLManager::SocInit(uint8_t *bdaddr, bool is_emb_wp_mode)
{
  ...
  switch (chipset_ver_) {
    ...
    case CHEROKEE_VER_1_0:
      ...
      if (!stat(CHEROKEE_RAMPATCH_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_1_0_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case CHEROKEE_VER_1_1:
      ...
      if (!stat(CHEROKEE_RAMPATCH_TLV_1_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_1_1_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_1_1_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_1_1_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_1_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_1_1_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_1_1_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case CHEROKEE_VER_2_0_1:
      ...
      if (!stat(CHEROKEE_RAMPATCH_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_2_0_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case CHEROKEE_VER_2_1_1:
      ...
      if (!stat(CHEROKEE_RAMPATCH_TLV_2_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_2_1_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_2_1_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_2_1_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_2_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_2_1_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_2_1_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case CHEROKEE_VER_3_1:
      ...
      if (!stat(CHEROKEE_RAMPATCH_TLV_3_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_3_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_3_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_3_0_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_3_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_3_0_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_3_0_PATH_ALT, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_3_0_PATH_ALT;
      }
      break;
    case CHEROKEE_VER_3_2:
      rampatch_file_path = CHEROKEE_RAMPATCH_TLV_3_2_PATH;
      nvm_file_path = CHEROKEE_NVM_TLV_3_2_PATH;

      if (!stat(CHEROKEE_RAMPATCH_TLV_3_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_3_2_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_RAMPATCH_TLV_3_2_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = CHEROKEE_RAMPATCH_TLV_3_2_PATH_ALT;
      }
      if (!stat(CHEROKEE_NVM_TLV_3_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_3_2_PATH_ALT_QTI;
      } else if (!stat(CHEROKEE_NVM_TLV_3_2_PATH_ALT, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = CHEROKEE_NVM_TLV_3_2_PATH_ALT;
      }
      break;
    ...
    case NAPIER_VER_1_0_1:
      ...
      if (!stat(NAPIER_RAMPATCH_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = NAPIER_RAMPATCH_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(NAPIER_RAMPATCH_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = NAPIER_RAMPATCH_TLV_1_0_PATH_ALT;
      }
      if (!stat(NAPIER_NVM_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = NAPIER_NVM_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(NAPIER_NVM_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case NAPIER_VER_2_0:
      ...
      if (!stat(NAPIER_RAMPATCH_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = NAPIER_RAMPATCH_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(NAPIER_RAMPATCH_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = NAPIER_RAMPATCH_TLV_2_0_PATH_ALT;
      }
      if (!stat(NAPIER_NVM_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = NAPIER_NVM_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(NAPIER_NVM_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case ROME_VER_3_0:
      ...
      if (!stat(ROME_RAMPATCH_TLV_3_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = ROME_RAMPATCH_TLV_3_0_0_PATH_ALT_QTI;
      } else if (!stat(ROME_RAMPATCH_TLV_3_0_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = ROME_RAMPATCH_TLV_3_0_0_PATH_ALT;
      }
      if (!stat(ROME_NVM_TLV_3_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = ROME_NVM_TLV_3_0_0_PATH_ALT_QTI;
      } else if (!stat(ROME_NVM_TLV_3_0_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case ROME_VER_3_2:
      ...
      if (!stat(ROME_RAMPATCH_TLV_3_0_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = ROME_RAMPATCH_TLV_3_0_2_PATH_ALT_QTI;
      } else if (!stat(ROME_RAMPATCH_TLV_3_0_2_PATH_ALT, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = ROME_RAMPATCH_TLV_3_0_2_PATH_ALT;
      }
      if (!stat(ROME_NVM_TLV_3_0_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = ROME_NVM_TLV_3_0_2_PATH_ALT_QTI;
      } else if (!stat(ROME_NVM_TLV_3_0_2_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_0_0_5:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_0_0_5_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_0_0_5_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_0_0_5_PATH_ALT, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = APACHE_RAMPATCH_TLV_0_0_5_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_0_0_5_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_0_0_5_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_0_0_5_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_0_0_6:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_0_0_6_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_0_0_6_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_0_0_6_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_0_0_6_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_0_0_6_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_0_0_6_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_0_0_6_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_1_0_0:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_1_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_0_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_1_0_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_0_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_1_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_1_0_0_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_1_0_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_1_0_1:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_1_0_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_1_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_1_0_1_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_1_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_1_0_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_1_0_1_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_1_0_1_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_1_0_2:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_1_0_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_2_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_1_0_2_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_2_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_1_0_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_1_0_2_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_1_0_2_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_1_0_3:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_1_0_3_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_3_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_1_0_3_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_0_3_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_1_0_3_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_1_0_3_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_1_0_3_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case APACHE_VER_1_2_1:
      ...
      if (!stat(APACHE_RAMPATCH_TLV_1_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_1_0_PATH_ALT_QTI;
      } else if (!stat(APACHE_RAMPATCH_TLV_1_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = APACHE_RAMPATCH_TLV_1_1_0_PATH_ALT;
      }
      if (!stat(APACHE_NVM_TLV_1_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = APACHE_NVM_TLV_1_1_0_PATH_ALT_QTI;
      } else if (!stat(APACHE_NVM_TLV_1_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case COMANCHE_VER_1_0:
      ...
      if (!stat(COMANCHE_RAMPATCH_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_RAMPATCH_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_0_PATH_ALT;
      }
     if (!stat(COMANCHE_NVM_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_NVM_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case COMANCHE_VER_1_0_1:
      ...
      if (!stat(COMANCHE_RAMPATCH_TLV_1_0_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_0_1_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_RAMPATCH_TLV_1_0_1_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_0_1_PATH_ALT;
      }
      if (!stat(COMANCHE_NVM_TLV_1_0_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_0_1_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_NVM_TLV_1_0_1_PATH_ALT, &sts) && sts.st_size > 0) {
    ...
    case COMANCHE_VER_1_1:
      ...
      if (!stat(COMANCHE_RAMPATCH_TLV_1_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_1_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_RAMPATCH_TLV_1_1_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_1_PATH_ALT;
      }
      if (!stat(COMANCHE_NVM_TLV_1_1_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_1_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_NVM_TLV_1_1_PATH_ALT, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_1_PATH_ALT;
      }
      break;
    case COMANCHE_VER_1_2:
      rampatch_file_path = COMANCHE_RAMPATCH_TLV_1_2_PATH;
      nvm_file_path = COMANCHE_NVM_TLV_1_2_PATH;

      if (!stat(COMANCHE_RAMPATCH_TLV_1_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_2_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_RAMPATCH_TLV_1_2_PATH_ALT, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = COMANCHE_RAMPATCH_TLV_1_2_PATH_ALT;
      }
      if (!stat(COMANCHE_NVM_TLV_1_2_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_2_PATH_ALT_QTI;
      } else if (!stat(COMANCHE_NVM_TLV_1_2_PATH_ALT, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = COMANCHE_NVM_TLV_1_2_PATH_ALT;
      }
      break;
    case HASTINGS_VER_1_0:
    case HASTINGS_VER_1_0_1:
    case HASTINGS_VER_1_1:
      rampatch_file_path = HASTINGS_RAMPATCH_TLV_1_0_PATH;
      nvm_file_path = HASTINGS_NVM_TLV_1_0_PATH;

      if (!stat(HASTINGS_RAMPATCH_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = HASTINGS_RAMPATCH_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(HASTINGS_RAMPATCH_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = HASTINGS_RAMPATCH_TLV_1_0_PATH_ALT;
      }

      if (!stat(HASTINGS_NVM_TLV_1_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
          nvm_alt_file_path = HASTINGS_NVM_TLV_1_0_PATH_ALT_QTI;
      } else if (!stat(HASTINGS_NVM_TLV_1_0_PATH_ALT, &sts) && sts.st_size > 0) {
          nvm_alt_file_path = HASTINGS_NVM_TLV_1_0_PATH_ALT;
      }
      break;
    case HASTINGS_VER_2_0:
      rampatch_file_path = HASTINGS_RAMPATCH_TLV_2_0_PATH;
      nvm_file_path = HASTINGS_NVM_TLV_2_0_PATH;

      if (!stat(HASTINGS_RAMPATCH_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = HASTINGS_RAMPATCH_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(HASTINGS_RAMPATCH_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
          rampatch_alt_file_path = HASTINGS_RAMPATCH_TLV_2_0_PATH_ALT;
      }

      if (!stat(HASTINGS_NVM_TLV_2_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
          nvm_alt_file_path = HASTINGS_NVM_TLV_2_0_PATH_ALT_QTI;
      } else if (!stat(HASTINGS_NVM_TLV_2_0_PATH_ALT, &sts) && sts.st_size > 0) {
          nvm_alt_file_path = HASTINGS_NVM_TLV_2_0_PATH_ALT;
      }
      break;
    default:
      ...
      if (!stat(TEMP_RAMPATCH_TLV_0_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        rampatch_alt_file_path = TEMP_RAMPATCH_TLV_0_0_0_PATH_ALT_QTI;
      ...
      if (!stat(TEMP_NVM_TLV_0_0_0_PATH_ALT_QTI, &sts) && sts.st_size > 0) {
        nvm_alt_file_path = TEMP_NVM_TLV_0_0_0_PATH_ALT_QTI;
  ...
  */
  ...
  if (soc_type_ == BT_SOC_CHEROKEE || soc_type_ == BT_SOC_HASTINGS) {
    if (!add_on_feat_mask_len) {
      ReadScramblingSupport();
    } else {
      char val[PROPERTY_VALUE_MAX] = { '\0' };
      if (AUDIO_SPLIT_A2DP_SCRAMBLING_DATA_SUPPORT(add_on_features.features)) {
        if (SPLIT_A2DP_44P1KHZ_FREQ(add_on_features.features)) {
          strlcat(val, "441.", PROPERTY_VALUE_MAX);
          strlcat(val, "882.", PROPERTY_VALUE_MAX);
        }
        if (SPLIT_A2DP_48KHZ_FREQ(add_on_features.features)) {
          strlcat(val, "48.", PROPERTY_VALUE_MAX);
          strlcat(val, "96.", PROPERTY_VALUE_MAX);
        }
      }

      if (!property_set("persist.vendor.bt.soc.scram_freqs", val)) {
        ALOGD("%s:persist.vendor.bt.soc.scram_freqs set to %s",
                       __func__, val);
      } else {
        // reset the property
    ...
    case EDL_CMD_REQ_RES_EVT:
      ...
      switch (subOpcode) {
        ...
        case HCI_VS_GET_BUILD_VER_EVT:
          ...
          memcpy(build_label, &rsp[bld_len_offset + 1], build_lbl_len);

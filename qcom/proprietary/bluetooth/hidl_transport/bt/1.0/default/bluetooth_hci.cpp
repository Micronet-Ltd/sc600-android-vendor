  ...
  const ::android::sp<IBluetoothHciCallbacks>& cb)
{
  ...
  bool rc = DataHandler::Init( TYPE_BT,
    [this](bool status) {
      if (event_cb_ != event_cb_tmp_ && event_cb_tmp_ != nullptr) {
        /* If ANT/FM already enabled BT SoC, we will
         * reach this point, so set the callback correctly to send
         * callback to relevant client for sending proper status
         */
        ALOGI("%s: Set callback correctly to the relevant client for sending "
                "proper status as ANT/FM has already enabled SoC", __func__);
        event_cb_ = event_cb_tmp_;
        event_cb_tmp_->linkToDeath(deathRecipient, 0);
      }
  ...
  } else if (rc && event_cb_ != cb && (cb != nullptr)) {
     /* BT client is valid, set callback pointer now. Status of
      * enablement would be broadcasted via the callback.
      */
     ALOGI("%s: Setting callback pointer as BT client is valid", __func__);
     event_cb_ = cb;

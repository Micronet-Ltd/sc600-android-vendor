...
void UartController::ReportSocFailure(bool dumped_uart_log, ReasonCode reason)
{
   ...
   * is due to Rx thread struck.
   */
  if (read_cb_ && reason != REASON_RX_THREAD_STRUCK) {
...
#ifdef USER_DEBUG
  /* Trigger abort for SoCWaitCrashTimeout Reasons */
  if (reason == REASON_SPURIOUS_WAKEUP_SOC_WAIT_TIMEOUT ||
      reason == REASON_SSR_INVALID_BYTES_RCVD ||
      reason == REASON_CMD_TIMEDOUT_SOC_WAIT_TIMEOUT) {
    ALOGE("%s: Aborting Daemon as SSR completion timedout!", __func__);
    abort();
  } else {

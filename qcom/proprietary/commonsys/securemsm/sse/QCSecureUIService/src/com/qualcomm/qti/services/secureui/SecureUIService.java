...
public class SecureUIService extends Service implements Runnable{
  ...
  public void run() {
    ...
    do {
      ...
      byte[] mex;
      //On tui_comm death, retry acquiring session from the new instance
      while((mex = waitForMessage(input)) == null){
        Log.w(TAG, " tui_comm is not alive. trying to acquire the session again!");
        getsession();
      }
...
void getsession(){
    ...
    do {
      // cleanup previous svc connection with tui_comm
      rv = terminate();
      if(rv == 0){
          rv = init();
          if(rv != 0){
              Log.d(TAG, "svc_sock_init failed rv = "+ rv);
              continue;
          }
          // Abort previous touch session if any
          sendNotification(SUI_MSG_ID_ABORT, SUI_MSG_RET_NOP, TOUCH_LIB_ADDR);
          Log.d(TAG, "getsessioni rv="+rv);
          break;
      }
      else {
          try {
  ...
  private int ProcessNotification(int id, int payload) {
    ...
    if (id == SUI_MSG_ID_SD_STOPPED) {
      ...
      //TODO Enable after display_config lib is available

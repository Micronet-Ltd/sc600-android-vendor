...
void LogCollectorReSetConfigHandler::HandleRequest(const LogCollectorReSetConfigRequest & jsonRequest, LogCollectorReSetConfigResponse & jsonResponse)
{
    ...
    OperationStatus os = Host::GetHost().GetDeviceManager().SubmitLogCollectorConfiguration("", CPU_TYPE_FW, logCollectorConfiguration);
    if (!os)
    {
        jsonResponse.Fail(os.GetStatusMessage());
    }

    os = Host::GetHost().GetDeviceManager().SubmitLogCollectorConfiguration("", CPU_TYPE_UCODE, logCollectorConfiguration);
    if (!os)
    {
        jsonResponse.Fail(os.GetStatusMessage());
    }

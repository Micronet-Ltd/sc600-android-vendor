...
#define MIN(a, b) ((a) < (b) ? (a) : (b))
    ...
    std::string LogTxtParser::ParseLogMessage(unsigned int stringOffset, const std::vector<int> &params,
    ...
    void LogTxtParser::ReadModuleNames()
    {
        ...
        for (int i = 0; i < MAX_MODULES; i++, mod = next_mod(mod)) {
            std::stringstream ssModuleName;
            ssModuleName << std::left;
            ssModuleName << "[" << std::setw(7) << mod << "]";
            m_moduleNames[i] = ssModuleName.str();
        }
        m_moduleNames[MAX_MODULES] = "[INVALID MODULE INDEX]";
    }

    const char * LogTxtParser::GetVerbosityLevelByIndex(int index) const
    {
        if (index >= MAX_VERBOS_LEVEL || index < 0)
        {
    ...
    const std::string& LogTxtParser::GetModuleNameByIndex(int index)
    {
        if (index >= MAX_MODULES || index < 0)
        {
            return m_moduleNames[MAX_MODULES];
        }
        ...
        for (;;) {
            ...
            */
            ...
            do {
                ...
                LOG_DEBUG << "stateMachineCurrentState: " << stateMachineCurrentState << "ps: " << ps << std::endl;
            }
            // stateMachineCurrentState is unsigned so if stateMachineCurrentState == 0 then stateMachineCurrentState-1 is very big therefore > STOP
            // so, once we got invalid state we stop.
            while (stateMachineCurrentState - 1 < STOP);
            ...
            case 'S':
                ...
                //case 'm':
            default:
                LOG_VERBOSE << "Invalid format specifier: " << t << std::endl;
                continue;
            }

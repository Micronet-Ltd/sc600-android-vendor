...
const string CommandHandler::s_version = TOOL_VERSION_STR;
...
{
    cout << "Shell 11ad version: " << s_version << endl;
}
...
bool CommandHandler::ValidateHostResponse(const string & response)
{
    ...
    if (response.empty())
    {
        LOG_ERROR << "host_manager_11ad version is not supported. Expected host_manager_11ad version: " << s_version << endl;
        return false;
    }
    ...
    if (!parsingSuccessful)
    {
        ...
        LOG_ERROR << "Expected host_manager_11ad version: " << s_version << endl;
        return false;
    }
    ...
    if (responseRoot["HostVersion"].asString().compare(s_version) != 0)
    {
        LOG_ERROR << "Unexpected host_manager_11ad. shell_11ad version is: " << s_version << " host_manager_11ad version on target is: " << responseRoot["HostVersion"].asString() << endl;

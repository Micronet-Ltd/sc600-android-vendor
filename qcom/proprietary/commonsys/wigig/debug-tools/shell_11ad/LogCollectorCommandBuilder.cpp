...
void LogCollectorCommandBuilder::SetOpcodeGenerators()
{
    ...
    RegisterOpcodeGenerators("set_deferred_mode", unique_ptr<LCSetDeferredModeGenerator>(new LCSetDeferredModeGenerator()));
...
LCSetDeferredModeGenerator::LCSetDeferredModeGenerator() :
    OpCodeGeneratorBase("Set log collector operation deferred mode: start and stop publishing and recording logs. \nIf parameters are set to true, host_manager will record any new interface that was discovered")
{
    RegisterParam(unique_ptr<ParameterObject>(new BoolParameterObject("PublishLogs", false, "Set Publish logs (raise event to DmTools).")));
    RegisterParam(unique_ptr<ParameterObject>(new BoolParameterObject("RecordLogs", true, "Set Record logs (write logs to file)")));
}
    ...
    OpCodeGeneratorBase("Set FW / UCODE module verbosity. \nV-verbose, I-information, E-error, W-warning")
{
    ...
    SetOpcodeCommonParams();
    RegisterParam(unique_ptr<ParameterObject>(new StringParameterObject("CpuType", true, "Select specific cpu.", "fw / ucode")));
    RegisterParam(unique_ptr<ParameterObject>(new StringParameterObject("DefaultVerbosity", false, "Set Verbosity level for all modules", "Any combination of {V, I, E, W}, e.g V, VI, IE .. or empty(\"\")", true)));

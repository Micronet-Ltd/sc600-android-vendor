...
public class QtiPhoneSwitcher extends PhoneSwitcher {
    ...
    private final int EVENT_RADIO_AVAILABLE   = 114;
    ...
    public QtiPhoneSwitcher(int maxActivePhones, int numPhones, Context context,
        ...
        for (int i = 0; i < mCommandsInterfaces.length; i++) {
            mCommandsInterfaces[i].registerForAvailable(this, EVENT_RADIO_AVAILABLE, i);
        }
        ...
        switch (msg.what) {
            ...
            case EVENT_RADIO_AVAILABLE:
                if (mQtiRilInterface.isServiceReady()) {
                    ...
                    for (int i = 0; i < mCommandsInterfaces.length; i++) {
                        mCommandsInterfaces[i].unregisterForAvailable(this);
                    }
                } else {
    ...
    protected int phoneIdForRequest(NetworkRequest netRequest, int apnid) {
        ...
        if (TextUtils.isEmpty(specifier)) {
            ...
            if ((APN_IMS_ID == apnid) && mManualDdsSwitch && mMaxActivePhones != mNumPhones) {
                subId = mDefaultDataSubscription;
            } else {
                try {
    ...
    protected void activate(int phoneId) {
        ...
        informDdsToRil(mDefaultDataSubscription);
        state.active = true;

...
public class MobileNetworkSettings extends Activity  {
    ...
    public static class MobileNetworkFragment extends PreferenceFragment implements
        ...
        private NwChangeObserver mNwChangeObserver;
        private boolean nwChangeObserverRegisted = false;
        ...
        private void updatePhone(int slotId) {
            ...
            if (mOnCertificationMode && TelephonyManager.getDefault().isVerizon()) {
                if (nwChangeObserverRegisted) {
                    getContext().getContentResolver().
                            unregisterContentObserver(mNwChangeObserver);
                    nwChangeObserverRegisted = false;
                }
                getContext().getContentResolver().registerContentObserver(
                        Settings.Global.getUriFor(Settings.Global.PREFERRED_NETWORK_MODE + mPhone.getSubId()),
                        false, mNwChangeObserver);
                nwChangeObserverRegisted = true;
            }
        ...
        public void onCreate(Bundle icicle) {
            ...
            log("mOnCertificationMode : " + mOnCertificationMode + ", isVerizon : " + isVerizon + ", mccmnc=" + mccmnc);
            if (mOnCertificationMode && isVerizon){
            ...
            mNwChangeObserver = new NwChangeObserver();
            Log.i(LOG_TAG, "onCreate:-");
        }
        ...
        private class NwChangeObserver extends ContentObserver {

            public NwChangeObserver() {
                super(null);
            }

            @Override
            public void onChange(boolean selfChange) {
                Log.i(LOG_TAG, "NwChangeObserver#onChange.");
                mNwChangeHandler.sendEmptyMessage(0);
            }

        }

        private Handler mNwChangeHandler = new Handler() {
            public void handleMessage(Message msg) {
                updateBody();
            }
        };
        ...
        private void updateBodyAdvancedFields(Activity activity, PreferenceScreen prefSet,
            ...
            if (ps != null) {
                ps.setEnabled(hasActiveSubscriptions && mPhone.getPhoneType()
                    ...
                    && !(mOnCertificationMode && TelephonyManager.getDefault().isVerizon()
        ...
        private boolean is4gLteEditableForNonDdsCtCard() {
            //just consider Daul SIM scenario for CT
            if (!getResources().getBoolean(R.bool.config_enhanced4glte_status)
            ...
            List<SubscriptionInfo> sirList =
                SubscriptionManager.from(mPhone.getContext()).getActiveSubscriptionInfoList();
            int defaultDataSub[] = SubscriptionManager.getSubId(dataPhoneId);
            boolean isDefaultCtCard = false;
            if (defaultDataSub != null && defaultDataSub.length > 0) {
                SubscriptionInfo subInfo =
                    SubscriptionManager.from(mPhone.getContext())
                    .getActiveSubscriptionInfo(defaultDataSub[0]);
                log("subInfo : " + subInfo);
                if (subInfo != null ) {
                    String iccId = subInfo.getIccId();
            ...
            boolean isCtCard = false;
            SubscriptionInfo subInfoNonDds =
                SubscriptionManager.from(mPhone.getContext()).getActiveSubscriptionInfo(subId);
                log("subInfo : " + subInfoNonDds);

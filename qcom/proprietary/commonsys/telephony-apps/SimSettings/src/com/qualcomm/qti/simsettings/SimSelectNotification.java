...
public class SimSelectNotification extends BroadcastReceiver {
    ...
    private static boolean haveShowNoSimDialog = false;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        final boolean mOnCertificationMode = SystemProperties.getBoolean("persist.certification.mode", false);
        ...
        if (numSlots < 2 || isInProvisioning ||
                !SystemProperties.getBoolean("persist.vendor.radio.aosp_usr_pref_sel", false) && !mOnCertificationMode) {
        ...
        for (int i = 0; i < numSlots; i++) {
            ...
            if (state != TelephonyManager.SIM_STATE_ABSENT) {
                Log.d(TAG, "[" + i + "]not absent, sim state = " + state);
                allSimAbsent = false;
            }
        }

        if (mOnCertificationMode) {
            if(allSimAbsent && !haveShowNoSimDialog) {
                haveShowNoSimDialog = true;
                Log.d(TAG, "allSimAbsent alert dialog!!!!");
                alertNoSIM(context);
            }
    ...
    private void alertNoSIM(Context context) {
        if (mNoSIMDialog != null) {
            mNoSIMDialog.dismiss();
            mNoSIMDialog = null;
        }
        AlertDialog.Builder builder = new AlertDialog
            .Builder(context).setTitle(R.string.no_sim_card)

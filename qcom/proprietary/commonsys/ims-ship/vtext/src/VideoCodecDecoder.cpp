...
void VideoCodecDecoder::videoPlayFrameLoop()
{
  ...
  while (eVideoStateActive == iPlayerState) {
    ...
    uint64_t currenttime = getCurrentSystemTime(NULL);
    ...
    if (status >= 0) {
      ...
      uint8_t ucCvoInfo;
      int8_t cIndex;
      int belongs_to_drop_set = DropFrameCheck(info.presentationTimeUs, ucCvoInfo,
                                cIndex, currenttime);
    ...
    int belongs_to_drop_set)
{
  ...
  MINIMAL4("VideoPlayFrame: input buffer %ld timestamp: %lu, dropset %d",
  ...
  if (bufidx >= 0) {
    ...
    if (buf && length < bufsize) {
      AddToFrameStateMap(pFrame, belongs_to_drop_set, ullCurTime * 1000);
  ...
  for (int i = 0; i < FRAME_STATE_SET_SIZE; i++) {
    if ((m_frameStateSet[i].app_tstamp_in_us == (uint64_t)app_tstamp) ||
      ...
      if (m_frameStateSet[i].belongs_to_drop_set) {
        ...
        playoutInfo.system_time_render_video_frame = getCurrentAVTime() / 1000;
      }

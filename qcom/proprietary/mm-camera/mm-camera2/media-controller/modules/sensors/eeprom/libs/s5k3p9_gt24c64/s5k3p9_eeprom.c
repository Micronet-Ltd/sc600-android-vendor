...
/** s5k3p9sx_eeprom_format_module_info:
 ...
 * Return: void.
 **/
static int s5k3p9_eeprom_format_module_info(sensor_eeprom_data_t* e_ctrl)
{
    uint8_t flag;
    int rc;
    ...
    SLOW("module_id=%d,%d-%d-%d",
         module_info->module_id,
         module_info->year,
         module_info->month,
         module_info->day);
    SLOW("lens_id = %d,driverIC_id = %d,vcm_id = %d\n",
        module_info->lens_id,
 ...
 * Return: void.
 **/
static int s5k3p9_eeprom_format_wbdata(sensor_eeprom_data_t *e_ctrl)
{
    unsigned char flag;
    int index = 0;
    ...
    if (!(buff->gb_over_gr_h || buff->gb_over_gr_l)){
        SERR("AWB Info:invaild gr_over_gb = %d\n",(uint16_t)buff->gb_over_gr_h << 8 | buff->gb_over_gr_l);
        return SENSOR_FAILURE;
    }
    gr_over_gb = (float)QVALUE/((uint16_t)buff->gb_over_gr_h << 8 | buff->gb_over_gr_l);

    SERR("AWB Info: r/gr=%f,b/gb=%f,gr/gb=%f\n",r_over_gr,b_over_gb,gr_over_gb);
...
/***
   if filter_arrangement is BGGR or BGRG, use gr_over_gb
   if filter_arrangement is RGGB or GRBG, use 1/gr_over_gb
   refer to document:80-N5126-1 U, page19
**/
    if(fabs(gr_over_gb) <= 1e-6){
       SERR("fatal! gr_over_gb is equal 0");
       return SENSOR_FAILURE;
    }
...
/** s5k3p9_eeprom_format_lensshading:
 *    @e_ctrl: point to sensor_eeprom_data_t of the eeprom device
 *
 ...
 * Return: void.
 **/
static int s5k3p9_eeprom_format_lensshading(sensor_eeprom_data_t *e_ctrl)
{
    unsigned char  flag;
    int index = 0;
    uint16_t i, light;
 ...
 * Return: void.
 **/
static int s5k3p9_eeprom_format_afdata(sensor_eeprom_data_t *e_ctrl)
{
    unsigned char     flag;
    af_data_t         *af;
    st_reg_addr       *af_addr;
 ...
 * Return: void.
 **/
static int s5k3p9_eeprom_format_pdafgain(sensor_eeprom_data_t *e_ctrl)
{
...
/** s5k3p9_eeprom_get_calibration_items:
 *    @e_ctrl: point to sensor_eeprom_data_t of the eeprom device
 *
...
/** s5k3p9_eeprom_format_calibration_data:
 *    @e_ctrl: point to sensor_eeprom_data_t of the eeprom device
 *
 ...
 * Return: void.
 **/
void s5k3p9_eeprom_format_calibration_data(void *e_ctrl) {
...
/** s5k3p9_gt24c64_eeprom_open_lib:
 *
 ...
 **/
void* s5k3p9_gt24c64_eeprom_open_lib(void) {
    SLOW("enter");
    return &s5k3p9_eeprom_lib_func_ptr;
}

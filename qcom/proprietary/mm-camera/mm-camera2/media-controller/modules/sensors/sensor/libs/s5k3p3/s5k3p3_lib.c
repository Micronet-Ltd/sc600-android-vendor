...
#include <stdio.h>
#include <string.h>
#include "s5k3p3_lib.h"
#include "debug_lib.h"
//#include <utils/Log.h>

/**
 * FUNCTION: s5k3p3_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int s5k3p3_real_to_register_gain(float real_gain)
{
...
/**
 * FUNCTION: s5k3p3_register_to_real_gain
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
static float s5k3p3_register_to_real_gain(unsigned int reg_gain)
{
    float real_gain;
...
/**
 * FUNCTION: s5k3p3_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int s5k3p3_digital_gain_calc(float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = s5k3p3_MIN_DGAIN_REG_VAL;
    float real_dig_gain = s5k3p3_MIN_DGAIN;
    ...
    {
        real_dig_gain = s5k3p3_MIN_DGAIN;
    }

    if(real_dig_gain > s5k3p3_MAX_DGAIN)
    {
        real_dig_gain = s5k3p3_MAX_DGAIN;
    }
...
/**
 * FUNCTION: s5k3p3_calculate_exposure
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    exp_info->reg_gain = s5k3p3_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain = s5k3p3_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      s5k3p3_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
...
/**
 * FUNCTION: s5k3p3_fill_exposure_array
 *
...
//  ALOGE("%s,fl_lines is %d,line is %d,gain is %d,%d\n",__func__,fl_lines,line,gain,__LINE__);
  ...
  //end
   if(line<108780){
	    ...
	    reg_count = reg_count + 1;
	   #if 1
	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_GR_ADDR;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_GR_ADDR + 1;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_R_ADDR;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_R_ADDR + 1;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_B_ADDR;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_B_ADDR + 1;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_GB_ADDR;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

	    reg_setting->reg_setting[reg_count].reg_addr = s5k3p3_DIG_GAIN_GB_ADDR + 1;
	    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
	   #endif
		...
		for (i = 0; i < sensor_lib_ptr.groupoff_settings.size; i++) {
   		...
   		temp=(double )line;
   		temp=(temp*5148-512)/(65523*8);
  		printf("%s,fl_lines is %d,line is %d,gain is %d,%f,%d\n",__func__,fl_lines,line,gain,temp,__LINE__);
		line=(unsigned int)(temp+0.5);
		fl_lines=line+16;
  		printf("%s,fl_lines is %d,line is %d,gain is %d,%d\n",__func__,fl_lines,line,gain,__LINE__);

		//page set 
   	    reg_setting->reg_setting[reg_count].reg_addr =0x6028;
	    reg_setting->reg_setting[reg_count].reg_data = 0x40;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
   	
   	     reg_setting->reg_setting[reg_count].reg_addr =0x6029;
	    reg_setting->reg_setting[reg_count].reg_data = 0x00;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
		
   		//stream off
   	    reg_setting->reg_setting[reg_count].reg_addr =0x0100;
	    reg_setting->reg_setting[reg_count].reg_data = 0x00;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
   		//linelength shifter
   	     reg_setting->reg_setting[reg_count].reg_addr =0x305a;
	    reg_setting->reg_setting[reg_count].reg_data = 0x00;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
		
	    reg_setting->reg_setting[reg_count].reg_addr =0x305b;
	    reg_setting->reg_setting[reg_count].reg_data = 0x03;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1; 
	   //fine integration time,fix value 
	    reg_setting->reg_setting[reg_count].reg_addr =0x0200;
	    reg_setting->reg_setting[reg_count].reg_data = 0x02;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
		
	   reg_setting->reg_setting[reg_count].reg_addr =0x0201;
	    reg_setting->reg_setting[reg_count].reg_data = 0x00;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
	   //fix line_lengh
	    reg_setting->reg_setting[reg_count].reg_addr =0x0342;
	    reg_setting->reg_setting[reg_count].reg_data = 0xff;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;
		
	   reg_setting->reg_setting[reg_count].reg_addr =0x0343;
	    reg_setting->reg_setting[reg_count].reg_data = 0xf3;
	    reg_setting->reg_setting[reg_count].delay = 0;
	    reg_count = reg_count + 1;

...
#include <stdio.h>
#include <string.h>
#include "s5k3p9_lib.h"
#include "debug_lib.h"

/**
 * FUNCTION: s5k3p9_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static uint32_t s5k3p9_real_to_register_gain(float real_gain)
{
  uint32_t reg_gain = 0;
...
/**
 * FUNCTION: s5k3p9_register_to_real_gain
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
static float s5k3p9_register_to_real_gain(uint32_t reg_gain)
{
  float real_gain;
...
static uint32_t s5k3p9_digital_gain_calc(float real_gain, float sensor_real_gain)
{
  uint32_t reg_dig_gain = S5K3P9_MIN_DGAIN_REG_VAL;
  float real_dig_gain = S5K3P9_MIN_DGAIN;

  if(real_gain > 16.0){
    real_dig_gain = real_gain / sensor_real_gain;
  }
  else {
    real_dig_gain = S5K3P9_MIN_DGAIN;
  }

  if(real_dig_gain > S5K3P9_MAX_DGAIN) {
    real_dig_gain = S5K3P9_MAX_DGAIN;
  }
  reg_dig_gain = (uint32_t)(real_dig_gain * 256);
  return reg_dig_gain;
}
/**
 * FUNCTION: s5k3p9_calculate_exposure
 *
 * DESCRIPTION: Calcuate the sensor exposure
 **/
  ...
  unsigned int line_count, sensor_exposure_info_t *exp_info, float s_real_gain)
{
  ...
  exp_info->reg_gain = s5k3p9_real_to_register_gain(real_gain);
  exp_info->sensor_real_gain = s5k3p9_register_to_real_gain(exp_info->reg_gain);
  exp_info->sensor_digital_gain =
    s5k3p9_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
...
/**
 * FUNCTION: s5k3p9_fill_exposure_array
 *

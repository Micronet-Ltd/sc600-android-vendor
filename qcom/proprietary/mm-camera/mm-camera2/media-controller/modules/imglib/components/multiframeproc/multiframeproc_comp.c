...
void multi_frameproc_mgr_check_pcr_mono(multi_frameproc_mgr_t *p_mgr) {
  ...
  IDBG_MED("master frame id %d valid %d slave frame_id %d valid %d",
    p_master_ab->frame_id,p_master_ab->valid,
  ...
  img_frame_bundle_t *p_super_bundle)
{
  ...
  for (i = 0; i < MFC_MAX_CAMERA_SESSIONS; i++) {
    ...
    IDBG_MED("is_master %d abl.min_fidx %d frameid %d",p_comp->is_master,
  ...
  if (can_create_sb) {
    ...
    IDBG_MED("process_frame_id %d",p_ab_master->frame_id);

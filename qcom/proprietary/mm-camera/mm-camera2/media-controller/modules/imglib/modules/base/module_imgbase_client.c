  ...
  int8_t buf_done)
{
  ...
  if (p_stream->p_srcport && p_mod->caps.use_internal_bufs) {
    ...
    IDBG_MED("[%s] [IMG_BUF_DBG_OUT_f%d_b%d_i%x] release fd %d buf_done %d",
      p_mod->name, frame_id, p_frame->idx, identity,
...
int module_imgbase_client_handle_framedrop(module_imgbase_t *p_mod,
  imgbase_client_t *p_client, uint32_t frame_id, uint32_t identity)
{
  struct msm_buf_mngr_info buff_info;
  int32_t cmd;
  int ret;
  ...
  IDBG_MED("[%s] identity %x frame id %d", p_mod->name, identity,
    frame_id);

  index = module_imgbase_client_get_and_update_buf_index(p_mod, p_client,
    frame_id, 1);
  if (index == 0xFFFFFFFF) {
    IDBG_ERROR("[%s] framedrop not required identity %x frame id %d",
      p_mod->name, identity, frame_id);
    return IMG_ERR_GENERAL;
  }
  ...
  if (ret < 0) {
    IDBG_ERROR("%s:%d] Failed to do buf_done id 0x%x %d %d",
      __func__, __LINE__, identity, out_idx, frame_id);
    return IMG_ERR_GENERAL;
  }

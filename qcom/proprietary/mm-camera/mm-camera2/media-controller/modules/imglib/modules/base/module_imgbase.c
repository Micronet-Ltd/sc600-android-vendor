  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_FRAME_DROP_NOTIFY: {
      ...
      IDBG_ERROR("[%s] FRAME_DROP_NOTIFY identity %x frame id %d", p_mod->name,
      ...
      if (p_mod->hal_version == CAM_HAL_V3 &&
        ...
        rc = module_imgbase_process_super_params(port, event,
          FALSE, p_mod_event->current_frame_id);
        if (IMG_ERROR(rc)) {
          IDBG_ERROR("[%s] cannot process super params rc %d",
            p_mod->name, rc);
        }
        rc = module_imgbase_client_handle_framedrop(p_mod, p_client,
          p_mod_event->current_frame_id, p_client->divert_identity);
      }
      break;
    }
    case MCT_EVENT_MODULE_BUF_DIVERT: {
      ...
      if (CAM_HAL_V1 == p_mod->hal_version ||
        ...
        for (i = 0; i < p_session_data->stream_id_requested[q_idx].num_streams; i++) {
          if (p_session_data->stream_id_requested[q_idx].stream_request[i].streamID ==
            ...
            if (p_mod->feature_mask & CAM_QTI_FEATURE_SAT) {
              IDBG_MED("MasterSwitch prev_master_sessid %d curr_sessid %d",
                p_mod->previous_master_sessionid, p_client->session_id);
              if (p_mod->previous_master_sessionid != p_client->session_id) {
                p_mod->modparams.imgbase_handle_master_switch(p_client,
                  CAM_MODE_PRIMARY);
                p_mod->previous_master_sessionid = p_client->session_id;
              }
            }
        ...
        if (p_mod->caps.dual_camfeature && !process_frame) {
          process_frame = TRUE;
        }
      }
       ...
       IDBG_MED("[%s] [IMG_BUF_DBG_IN_f%d_b%d_i%x] event identity %x"
        ...
        p_buf_divert->identity,
        p_client->divert_identity,
        p_client->streams_to_process,process_frame);

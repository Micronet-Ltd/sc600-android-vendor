  ...
  uint32_t identity)
{
  ...
  if (rc < 0) {
    CPP_BUF_DBG("No PCR for frame id %d",frame_id);
    return 0;
  }
  if (!session_params || !stream_params) {
  ...
  if (stream_frame_valid) {
    ...
    CPP_BUF_DBG("ISP buf drop for id %x frame %d \n",
      stream_params->identity, frame_id);
  ...
  boolean *param_processed, uint32_t current_frame_id)
{
  ...
  case CAM_INTF_META_STREAM_INFO : {
    ...
    ctrl->rtb_status =
      ((meta_stream_info->postprocess_mask[0] & CAM_QTI_FEATURE_RTB) ? 1 : 0);
    ctrl->sat_status =
      ((meta_stream_info->postprocess_mask[0] & CAM_QTI_FEATURE_SAT) ? 1 : 0);
    break;
  }

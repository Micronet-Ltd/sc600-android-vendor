...
int usb_fd = -1;
int mdm_fd = -1;
extern boolean wifi_modem_is_ture;
...
*/
/*==========================================================================*/
extern int quec_atg_handdler(void *buf);
      ...
      if (num_read > 0)
      {
        ...
        LOGI("buf contains '%s'\n", buf);
  		if(wifi_modem_is_ture==FALSE){
			...
			LOGI("USB Attempting to write to '%s'....\n", tinfo.from_file_path);//geoff
			LOGI("USB Attempting to fd to '%d'...\n", *tinfo.from_fd);
			ret=quec_atg_handdler(buf);

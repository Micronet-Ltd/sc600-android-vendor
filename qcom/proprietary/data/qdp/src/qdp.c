        ...
        case QDP_RIL_NAI:
          ...
          if(!prof_params->username_valid)
          {
            QDP_LOG_DEBUG("%s","modem profile parameter username not available");
                               ...
                               std_strlen(prof_params->username)) != 0)
          {
        ...
        case QDP_RIL_PASSWORD:
          ...
          if(!prof_params->password_valid)
          {
            QDP_LOG_DEBUG("%s","modem profile parameter password not available");
                               ...
                               std_strlen(prof_params->password)) != 0)
          {
  ...
  do
  {
    ...
    for (i=0; i<QDP_RIL_PARAM_MAX; i++)
    {
      ...
      case QDP_RIL_NAI:
        ...
        {
          if( strncmp( params[i].buf, resp.username, params[i].len))
          {
      ...
      case QDP_RIL_PASSWORD:
        ...
        {
          if( strncmp( params[i].buf, resp.password, params[i].len))
          {
...
)
{
  ...
  *error = QDP_ERROR_NONE;
  ...
  {
    QDP_LOG_DEBUG("%s: qdp roaming IP is v4v6, lte/umts profile has ip type other than V4V6", __func__);
    *error = QDP_ERROR_MAX;
    return FALSE;
  }
...
)
{
  ...
  if( mcc && mnc )
  {
    for(unsigned int i=0;i < QCRIL_DATA_MCC_MNC_LIST;i++)
    {

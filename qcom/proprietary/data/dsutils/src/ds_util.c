...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2008, 2010-2011, 2013-2019 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define DS_DEVICE_RMNET_USB "rmnet_usb"
...
)
{
  ...
  *epid = DS_EP_ID_INVALID;
  ...
  else if (!strncmp(DS_DEVICE_RMNET_USB, net_dev, strlen(DS_DEVICE_RMNET_USB)))
  {
    *ep_type = DS_EP_TYPE_HSUSB;
    *epid = DS_EP_ID_USB;
  }
...
static ds_target_t ds_get_target_from_mdmdetect()
{
 ...
 else if (devinfo.num_modems == 2)
 {
    ...
    {
      if ((devinfo.mdm_list[i].type == MDM_TYPE_EXTERNAL) &&
      (std_stricmp(devinfo.mdm_list[0].mdm_name, DS_BASEBAND_PROP_MDM ) == 0) &&
      (ds_get_target_from_soc_id() == DS_TARGET_TALOS_AU))
      {
         target = DS_TARGET_TALOS_AU;
         break;
      }
    }
 }
      ...
      else
      {
        ...
        switch((int)soc_id)
        {
          ...
          case DS_TARGET_SOC_ID_MSMNILE:
            ...
            break;
          case DS_TARGET_SOC_ID_TALOS_AU:
             target = DS_TARGET_TALOS_AU;
             break;

...
)
{
  ...
  memset( cmd, 0x0, sizeof(cmd) );
  snprintf(cmd, sizeof(cmd), "%s %s -t mangle -N %s_0x%08x.%d",
          ipt_tool,
          iptables_opts,
          NETMGR_TC_CHAIN_NAME_PREFIX,
          flow_id,
  ...
  snprintf(cmd, sizeof(cmd), "%s %s -t mangle -A %s_0x%08x.%d -j MARK --set-mark 0x%08x",
          ipt_tool,
          iptables_opts,
          NETMGR_TC_CHAIN_NAME_PREFIX,
          flow_id,
...
)
{
  ...
  memset( cmd, 0x0, sizeof(cmd) );
  length = (unsigned int)snprintf(cmd, maxlength, "%s %s -t mangle -I %s %d -g %s_0x%08x.%d",
...
)
{
  ...
  length = (unsigned int)snprintf(cmd,
                    ...
                    "htb prio %d rate %lubit ceil %umbit",
                    dev_name,
...
)
{
  ...
  else
  {
    ...
    else
    {
        ...
        {
            netmgr_log_err("flow %x not found in QoS modify cache", qos_flow->flow_id);
        }
      ...
      if (TRUE == recovery_data.is_valid && TYPE_BEARER_INFO == recovery_data.info_type)
      {
        ...
        while (filter_mask != 0)
        {
          ...
          if (filter_mask & 0x1)
          {
            ...
            snprintf(cmd, sizeof(cmd), "%s %s -t mangle -F %s_0x%08x.%d",
                     ipt_tool, iptables_opts, NETMGR_TC_CHAIN_NAME_PREFIX,
...
int netmgr_tc_create_delete_dynamic_post_routing_rule(int ip_family)
{
  ...
  length = snprintf(cmd,
                    maxlength,
                    "%s %s -t mangle -A %s -o rmnet_data+ -j MARK --set-mark 0x%x",
                    ipt_tool,
                    iptables_opts,
                    NETMGR_TC_CHAIN_NAME_QOS_RESET_POSTROUTING,

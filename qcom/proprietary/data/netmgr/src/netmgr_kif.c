...
netmgr_kif_set_mtu(int link, boolean post_nl_ev)
{
  ...
  else
  {
    /* If the config item is set to zero then we default to the old value of
       max_mtu which is 2000 */
    max_mtu = NETMGR_DEFAULT_MTU_MAX;
  }
#endif /* FEATURE_MSMNILE */
...
)
{
  ...
  if (link >= NETMGR_LINK_RMNET_0 && link <= (NETMGR_LINK_RMNET_0 + netmgr_main_cfg.num_data_ports - 1)) {
    *modem = NETMGR_MODEM_MSM;
    *link_type = NETMGR_FWD_LINK;
  }
  else if (link >= (NETMGR_LINK_RMNET_11)&&
          link <= (NETMGR_LINK_RMNET_11 + netmgr_main_cfg.num_data_ports - 1)) {
    *modem = NETMGR_MODEM_MDM;
    *link_type = NETMGR_FWD_LINK;
  }
#ifdef FEATURE_DATA_IWLAN
  else if (link >= NETMGR_LINK_REV_RMNET_0 && link <= (NETMGR_LINK_REV_RMNET_0 + netmgr_main_cfg.num_rev_data_ports - 1)) {
    *modem = NETMGR_MODEM_MSM;
    *link_type = NETMGR_REV_LINK;
  }
  else if (link >= (NETMGR_LINK_REV_RMNET_9)
          && link <= (NETMGR_LINK_REV_RMNET_9 + netmgr_main_cfg.num_rev_data_ports - 1)) {
    *modem = NETMGR_MODEM_MDM;
    *link_type = NETMGR_REV_LINK;
  }
#endif
...
netmgr_kif_nl_recv_link_msg (const netmgr_nl_msg_t  * nlmsg_info_ptr)
{
  ...
  if ((link = netmgr_kif_get_link_for_ifindex((uint32)ifinfo->ifi_index)) < 0 ) {
    ...
    ** care about. */
    ...
    netmgr_log_low(" %s(): is_gro_update_req %d clat_iface_st %d"
    ...
    if (is_gro_perf_update_req && (ifinfo->ifi_index >= 0) && netmgr_main_cfg.rmnet_perf) {
      ...
      if (clat_iface_st)
        ...
        perf_value = RMNET_PERF_OPT_MODE_ALL;

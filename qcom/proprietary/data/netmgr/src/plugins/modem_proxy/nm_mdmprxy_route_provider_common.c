...
{
  {
    "%s %s -t mangle -A nm_mdmprxy_pkt_marker -m mark --mark 0x%x"
    " -j nm_mdmprxy_pkt_forwarder",
    "%s %s -t mangle -D nm_mdmprxy_pkt_marker -m mark --mark 0x%x"
    " -j nm_mdmprxy_pkt_forwarder",
  ...
  { "%s %s -t mangle -A nm_mdmprxy_pkt_marker ! -p %s -m mark --mark 0x%x -j DROP",
    "%s %s -t mangle -D nm_mdmprxy_pkt_marker ! -p %s -m mark --mark 0x%x -j DROP",
    nm_mdmprxy_mark_drop_formatter },

  /* Remove marking from iWLAN marked packets to prevent confusion with QoS marking */
  { "%s %s -t mangle -A nm_mdmprxy_mngl_post -m mark --mark 0x%x"
                          " -j MARK --set-mark 0x0",
    "%s %s -t mangle -D nm_mdmprxy_mngl_post -m mark --mark 0x%x"
                          " -j MARK --set-mark 0x0",
    nm_mdmprxy_markjump_formatter }
};
...
static const struct nm_mdmprxy_cmd local_breakout_rule_list[] =
{
  ...
  { "%s %s -t mangle -I nm_mdmprxy_mngl_pre_tee -j MARK --set-mark " TOSTR(NM_MDMPRXY_LBO_MARK),
    "%s %s -t mangle -D nm_mdmprxy_mngl_pre_tee -j MARK --set-mark " TOSTR(NM_MDMPRXY_LBO_MARK),
    nm_mdmprxy_basic_formatter },
...
{
  { "%s %s -t filter -I nm_mdmprxy_doze_mode_skip -m mark --mark 0x%x -j ACCEPT",
    "%s %s -t filter -D nm_mdmprxy_doze_mode_skip -m mark --mark 0x%x -j ACCEPT",
...
)
{
  ...
  if (iiface_refcount == 0)
  {
    ...
    if (NETMGR_SUCCESS != iiface_handle)
    {

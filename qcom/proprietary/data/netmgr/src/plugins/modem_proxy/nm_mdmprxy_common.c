...
static const struct nm_mdmprxy_cmd static_rule_list[] =
{
  ...
  { "%s %s -t mangle -N nm_mdmprxy_mngl_pre",
    "%s %s -t mangle -X nm_mdmprxy_mngl_pre",
    nm_mdmprxy_basic_formatter },
...
static const struct nm_mdmprxy_cmd tcp_mss_rule_list[] =
{
  ...
  { "%s %s -t mangle -I nm_mdmprxy_mngl_pre -d %s -p tcp"
                          " --tcp-flags SYN,ACK SYN,ACK"
                          " -j TCPMSS --set-mss %d",
    "%s %s -t mangle -D nm_mdmprxy_mngl_pre -d %s -p tcp"
                          " --tcp-flags SYN,ACK SYN,ACK"
                          " -j TCPMSS --set-mss %d",
    nm_mdmprxy_tcp_mss_formatter }
};
...
static const struct nm_mdmprxy_cmd cleanup_tables_rule_list[] =
{
  ...
  { "%s %s -t filter -F nm_mdmprxy_mngl_pre",
    NULL,
    nm_mdmprxy_basic_formatter },
  ...
  { "%s %s -t filter -X nm_mdmprxy_mngl_pre",
    NULL,
    nm_mdmprxy_basic_formatter }
};
...
)
{
  ...
  for (int i = 0; i < nm_mdmprxy_rules_len(tcp_mss_rule_list); i++)
  {

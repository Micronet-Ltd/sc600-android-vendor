...
extern int remove_download_flag();
extern void show_update_complete_dialog();
...
and PAL_NOTIFY_STOP_NETWORK_CONDITIONS_CHANGES from %s",
    ...
    rc = stat(MO_WORK_PATH FUMO_STATE_DIR PAL_FILENAME_LAST_UPD_DATETIME,
        &file_stat);
	DM_LOGE(" start restore rc = %d   st_size = %ld\n", rc, (long)file_stat.st_size);
    if(-1 == rc || 0 == file_stat.st_size) {
        ...
        DM_LOGE(" alpha file name %s .\n",MO_WORK_PATH FUMO_STATE_DIR PAL_FILENAME_LAST_UPD_DATETIME);
        int ret = chmod(MO_WORK_PATH FUMO_STATE_DIR PAL_FILENAME_LAST_UPD_DATETIME,0777);
        DM_LOGE(" alpha ret : %d.\n",ret);
        time ( &rawtime );
        ptm = gmtime ( &rawtime );
...
//barret
    ...
    if(-1 == rc || 0 == file_stat.st_size) {
        ...
        chmod(MO_WORK_PATH FUMO_STATE_DIR PAL_FILENAME_LAST_UPD_FIRMWARE,0777);
    ...
    if(-1 == rc || 0 == file_stat.st_size) {
        ...
        chmod(MO_WORK_PATH FUMO_STATE_DIR PAL_FILENAME_FIRMWARE_PACKET_NAME,0777);

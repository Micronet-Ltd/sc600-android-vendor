...
extern int scm_set_job_state_in_queue(int state, int job_key);
extern int scm_pop_job_from_queue(const int job_key);
extern int scm_get_job_from_queue(const char *node, scm_job_description *info);
...
{
    if(ptr!=NULL)DM_LOGI("job_d_routine ...");
...
{
    if(ptr!=NULL)DM_LOGI("job_di_routine ...");
    scm_job_description *info = calloc(1, sizeof(scm_job_description));
...
{
    if(ptr!=NULL)DM_LOGI("job_diia_routine ...");
    scm_job_description *info = calloc(1, sizeof(scm_job_description));

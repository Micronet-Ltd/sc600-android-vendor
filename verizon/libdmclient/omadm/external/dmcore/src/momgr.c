                         ...
                         uint16_t * resultP)
{
    int error;
    dmtree_node_t node;
	DM_LOGI("abing prv_get_short plugin->interface = %s\n", plugin->interface->base_uri);
...
int momgr_check_mandatory_mo(mo_mgr_t * iMgrP)
{
	...
	DM_LOGI("abing momgr_check_mandatory_mo was used\n");
    prv_findUrn(iMgrP->root, URN_MO_DMACC, &urlList);
    if (NULL == urlList)
    {
        // missing DMAcc MO
		DM_LOGI("abing momgr_check_mandatory_mo find urn mo dmacc failed\n");
    ...
    {
        // missing DevInfo MO
		DM_LOGI("abing momgr_check_mandatory_mo find dev info failed\n");
        return OMADM_SYNCML_ERROR_COMMAND_FAILED;
    }
    ...
    if (NULL == pluginP)
    {
		...
		DM_LOGI("abing momgr_check_mandatory_mo find dev detail failed\n");
        return OMADM_SYNCML_ERROR_COMMAND_FAILED;
    }
                    ...
                    dmtree_node_t * nodeP)
{
    ...
    DM_LOGI("momgr_get_value iMgr->last_uri %s  nodeP->uri : %s ",iMgr->last_uri,nodeP->uri);
        ...
        if (dirP)
        {
            ...
            while (NULL != child)
            {
                if (NULL == nodeP->data_buffer)
                {
    				...
    				DM_LOGI("abing momgr_get_value null child->name = %s\n", child->name);
                ...
                else
                {
    				...
    				DM_LOGI("abing momgr_get_value nodeP->data_buffer = %s child->name = %s\n", nodeP->data_buffer, child->name);
                       ...
                       char ** oUri)
{
	...
	DM_LOGI("abing momgr_find_subtree was used\n");
    ...
    while(NULL != urlList[i] && false == found)
    {
		...
		DM_LOGI("abing data buffer = %s  iCriteriaValue = %s\n", node.data_buffer, iCriteriaValue);

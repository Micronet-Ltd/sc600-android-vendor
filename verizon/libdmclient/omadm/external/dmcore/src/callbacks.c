...
// Additional info for FUMO 403 status
#define FUMO_MSG_UPDATE_DEFERRED    "UPDATE_DEFERRED"
#define FUMO_MSG_DOWNLOAD_DEFERRED  "DOWNLOAD_DEFERRED"
#define FUMO_MSG_LOW_BATTERY        "LOW_BATTERY"
#define FUMO_MSG_WIFI_DEFERRED      "WIFI_DEFERRED"
#define FUMO_MSG_KEY_DEFERRED       "KEY_DEFERRED"
#define FUMO_MSG_LOW_MEMORY         "LOW_MEMORY"
                                  ...
                                  VoidPtr_t userData)
{
    ...
    smlFreeAtomic((SmlAtomicPtr_t)internP->sequence);
    internP->sequence = NULL;
    internP->seq_code = 0;
                             ...
                             SmlExecPtr_t execP)
{
    ...
    while (itemCell)
    {
        ...
        {
            code = OMADM_SYNCML_ERROR_NOT_EXECUTED;
            // Verizon Device Certification Update
            internP->seq_code = OMADM_SYNCML_ERROR_SUCCESS;
        }
        else
        {
        ...
        // if it is a FUMO 403 extended status, add item with corresponding message
        if(code >= FUMO_RC_PRV_DEFERRED_MIN &&
                code <= FUMO_RC_PRV_DEFERRED_MAX) {
            code_cpy = code;
            code  = FUMO_RC_UPDATE_DEFERRED;
        }
        statusP = create_status(internP, code, (SmlGenericCmdPtr_t)execP);

        switch(code_cpy) {
        case FUMO_RC_PRV_DOWNLOAD_DEFERRED:
        case FUMO_RC_PRV_DOWNLOAD_UPDATE_DEFERRED:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                     smlString2Pcdata(FUMO_MSG_DOWNLOAD_DEFERRED);
            break;
        case FUMO_RC_PRV_UPDATE_DEFERRED:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                   smlString2Pcdata(FUMO_MSG_UPDATE_DEFERRED);
            break;
        case FUMO_RC_PRV_UPDATE_DEFERRED_KEY_DEFERRED:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                   smlString2Pcdata(FUMO_MSG_KEY_DEFERRED);
            break;
        case FUMO_RC_PRV_UPDATE_DEFERRED_LOW_BATTERY:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                   smlString2Pcdata(FUMO_MSG_LOW_BATTERY);
            break;
        case FUMO_RC_PRV_UPDATE_DEFERRED_WIFI_DEFERRED:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                   smlString2Pcdata(FUMO_MSG_WIFI_DEFERRED);
            break;
        case FUMO_RC_PRV_UPDATE_DEFERRED_LOW_MEMORY:
            statusP->itemList = smlAllocItemList();
            if (statusP->itemList)
                statusP->itemList->item->data =
                                   smlString2Pcdata(FUMO_MSG_LOW_MEMORY);
            break;
        default:
            break;
        }

    ...
    if (pal_status == RESULT_SUCCESS) {
        ...
        if (pControllerGuiUserRP != NULL) {
            ...
            if (pControllerGuiUserRP->input_text) {
                free((void*)pControllerGuiUserRP->input_text);
                pControllerGuiUserRP->input_text = NULL;
            }
            if (pControllerGuiUserRP->selected_choices) {
                int i = 0;
                do {
                    free((void*)pControllerGuiUserRP->selected_choices[i]);
...
dmclt_err_t restart_session() {
    dmclt_err_t err = (dmclt_err_t)omadm_event_handler_process_event(START_AUTOSESSION, NULL);
...
dmclt_err_t emulate_su_cancel() {
    dmclt_err_t err = (dmclt_err_t)omadm_event_handler_process_event(SU_CANCEL, NULL);
    DM_LOGI("OMADM_C: call Event Handler process event. res = %d", err);
    return err;
}

dmclt_err_t user_update_session() {
    dmclt_err_t err = (dmclt_err_t)omadm_event_handler_process_event(USER_SYS_UPDATE, NULL);
    DM_LOGI("OMADM_C: call Event Handler process event. res = %d", err);
    return err;
}
...
bool omadm_controller_init()
{
    ...
    if (palHandle == NULL) {
        DM_LOGE("OMADM_C: ERR palHandle == NULL  %s ",dlerror());
    }
...
dmclt_err_t omadm_controller_start_session(omadm_request_t * omadm_request)
{
    ...
    if (PACKAGE0 == omadm_request->type) {
        ...
        if (DMCLT_ERR_NONE == status) {
            ...
            if(item == NULL)
                return (dmclt_err_t)MO_ERROR_DEVICE_FULL;

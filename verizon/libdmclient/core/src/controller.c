...
extern dmclt_err_t omadm_controller_handle_request(void * request);
...
void *dmc_queue_processor_thread(void *arg)
{
    ...
    while(1){
        if(dmc_events_queue_empty_lock()) {
            DM_LOGI("dmc_queue_processor_thread queue empty lock error");
            break;
        }
        request_t *req = dmc_events_queue_get();
        if (!req) {
            DM_LOGI("dmc_queue_processor_thread events queue get error");
            break;
        }
